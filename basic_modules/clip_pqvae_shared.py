...
#         super(Encoder, self).__init__()
#         self.initial_conv1 = nn.ConvTranspose2d(768, 512, kernel_size=3, stride=2, padding=0) # 1*1 -> 3*3
#         self.initial_conv2 = nn.ConvTranspose2d(512, 128, kernel_size=3, stride=2, padding=1) # 3x3 -> 5*5
#         self.residual_stack = ResidualStack(128, 128, 64, n_res_layers=3, inverse=True)  # output: 5*5
#         self.pre_quant_conv = nn.Conv2d(128, 64, kernel_size=4, stride=1, padding=1)
#         self.tanh = nn.Tanh()
#         self.relu = nn.ReLU()
...
#     def __init__(self):
#         super(Decoder, self).__init__()
#         self.post_quant_conv = nn.Conv2d(64, 128, kernel_size=1, stride=1, padding=1)  # Bx128x5*5 -> Bx256x5*5
#         self.residual_stack = ResidualStack(128, 128, 64, n_res_layers=3, inverse=False)
#         self.final_conv1 = nn.Conv2d(128, 512, kernel_size=3, stride=2, padding=1)  # Bx256x5*5 -> Bx512x3x3
#         self.final_conv2 = nn.Conv2d(512, 768, kernel_size=3, stride=2, padding=0)  # Bx512x3x3 -> Bx768x1x1
...
class VectorQuantizer(nn.Module):
    ...
    def __init__(self, n_e, num_bits, e_dim, beta, snr_db, device):
        ...
        self.embedding.weight.data.uniform_(-1.2, 1.2) # 除去(16-8)是1.2, 其他数据都来自1.0
    ...
    def forward(self, z):
        ...
        # huffman (extra)
        # tmp = [int(e) for e in min_encoding_indices.view(-1)]
